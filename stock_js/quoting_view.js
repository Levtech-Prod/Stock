var quoting_view = function(params){
    ...
    var print_record = function(record){
        ...
        crud_jsupdate('Quoting/list_parts',{quoting_id: record.id}, function(retData){
            ...
            row_html = row_html + '<div style="height: 40px; clear: both;"></div><div><b>Lead time:</b> '+(record.lead_time?record.lead_time:'0')+' business days after purchase order received</div>';
            //row_html = row_html + '<div style="height: 20px; clear: both;"></div><div>'+(record.description?(record.description).replace(/(?:\r\n|\r|\n)/g, '<br>'):'')+'</div>';

            row_html = row_html + '<div style="height: 20px; clear: both;"></div><div>'+(record.po_comment?'<b>Additional comment:</b> '+APP.utils.text.nl2br(record.po_comment):'')+'</div>';
            ...
            $img.closest('td'),
            {
                ...
                fields: {
                        ...
                        right_angle:{
                            ...
                            values: { '0': '', '1': '' },
                            listClass: 'text-center',
                            containerClass: 'jtabledlg-w80proc'
                        },
                        arm1:{
                ...
                formCreated: function (event, data) {
                    ...
                    $matuprice.dspinner({
                        ...
                        calc_material_prices($materialid, $width, $length, $height, $diameter,$quantity, $matprice, $matuprice, $total, 0, $cylinder, $right_angle, $arm1, $arm2);
                    });
                    ...
                    if (data.formType=='edit'){
                        calc_material_prices($materialid, $width, $length, $height, $diameter,$quantity, $matprice, $matuprice, $total, data.record.density, $cylinder, $right_angle, $arm1, $arm2);
                    }
    ...
    function calc_material_prices($materialid, $width, $length, $height, $diameter,$quantity, $matprice, $matuprice, $total, mat_density=0, $cylinder=0, $right_angle = 0, $arm1 = 0, $arm2 = 0){
        ...
        let quantity = parseFloat($quantity.val());
        let arm1 = parseFloat($arm1.val());
        let arm2 = parseFloat($arm2.val());
        let weight = 0;
        if($right_angle.prop('checked')){
