var orders_view = function(params){
        ...
        crud_jsupdate('Orders/list_jobs',{order_id: record.id}, function(retData){
            ...
            $.each(retData.Records, function(i, rec) {
                ...
                row_html = row_html + '<div style="display: inline-block; text-align: center; padding: 20px 10px 10px 0px; width:48%;"><table class="border_table">'+
                        '<tr><td class="bold">Projekt</td><td><b>'+rec.order_id+'</b></td></tr>'+
                        ...
                        '<tr><td class="bold">Nyersanyag</td><td>'+(rec.calibration==1?'(K) ':'')+(rec.height==0 && rec.width==0 && rec.length==0?'':(rec.cylinder==1?('D'+rec.diameter +' x '+rec.length+' mm'):(rec.height+' x '+rec.width+' x '+rec.length+' mm')))+'</td></tr>'+
    ...
    var print_materials = function(record, retData){
        ...
        $.each(retData.Records, function(r, rec) {
            i++;
            row_html = row_html + '<tr><td style="width:6%;">'+i+'</td><td style="width:8%;">'+rec.id+'</td><td style="width:10%;">'+rec.name+'</td><td style="width:10%;">'+rec.material_name+'</td><td style="width:15%;">'+(rec.cylinder==1?('D'+rec.diameter +' x '+rec.length+' mm'):(rec.height+' x '+rec.width+' x '+rec.length+' mm'))+' '+(rec.calibration==1?'(K) ':'')+'</td><td style="width:9%;">'+(rec.handling?rec.handling.replaceAll(',', ', '):'')+'</td><td style="width:9%;">'+rec.quantity+'</td><td style="width:21%;">'+(rec.order_comments?rec.order_comments:'')+'</td><td style="width:10%;"></td></tr>'+
    ...
    var orders_jobs = function($img, orderdata){
        $('#orders').jtable(
            ...
            $img.closest('td'),
            {
                ...
                formCreated: function (event, data) {
                    ...
                    */
                    ...
                    $matuprice.dspinner({
                        ...
                        calc_material_prices($materialid, $width, $length, $height, $diameter,$quantity, $matprice, $matuprice, $total, 0, $cylinder, $material_type);
                    });
                ...
                //if (data.formType=='create'){
                    ...
                    if (data.formType=='edit'){
                        /*if($material_type.val()==1){
                            ...
                            calc_material_prices($materialid, $width, $length, $height, $diameter,$quantity, $matprice, $matuprice, $total, data.record.density, $cylinder, $material_type);
                        //}
                    }
                ...
                rowInserted: function(event, data){
                    if ((parseInt(data.record.cylinder)==0 && (parseFloat(data.record.width)==0 || parseFloat(data.record.length)==0 || parseFloat(data.record.height)==0)) || (parseInt(data.record.cylinder)==1 && (parseFloat(data.record.diameter)==0 || parseFloat(data.record.length)==0))){
                        data.row.addClass('qpart_color');
                    }
    ...
    function calc_material_prices($materialid, $width, $length, $height, $diameter,$quantity, $matprice, $matuprice, $total, mat_density=0, $cylinder=0, $material_type = 0, $cut = null){
        ...
        /*if($material_type.val()==1){
            ...
            price = parseFloat($matuprice.val());
