var tool_in_view = function(params){
    $('#tool_stock').jtable({
            title: langJS('global_in'),
            ...
            fields: {
                ...
                username: {
                    title: langJS('global_took_out_user'),
                    width: '8%',
                    listClass: 'text-bold',
                    display: function(data){
    ...
    var build_item = function(item, button = true){
        var $categ = $('<div class="categ_item_container"><a style="display: block; height: 150px; cursor:pointer;" href="javascript:content_load(\'Tool_stock_categ\', {categ_id: '+item.id+', categ_name: \''+item.name+'\', edit: \'0\', title: \''+langJS('global_in')+'\', in: \'1\'});"><img style="width:100%; height: 100%;" src="'+base_url()+(item.image?'upload/images/'+item.image:'images/image_not_available.png')+'?'+ new Date().getTime()+'"></img></a></div>');
      return $categ;
    };
