        ...
        _onRecordsLoaded: function(data) {
            ...
            if( this.options.footer ){
                this._$tfoot.find('th').each(function (index, cell) {
                    ...
                    if( self.options.fields && self.options.fields[fieldName] && self.options.fields[fieldName].footer )
                    {
                        $cell.find('span')
                        .empty()
...
/*        _addColumnsToFooterRow: function ($tr) {
        ...
        _addColumnsToFooterRow: function ($tr) {
            ...
            for (var i = 0; i < this._columnList.length; i++) {
				...
				if ((this.options.fields[fieldName].footer) || (i==this._columnList.length-1)){
	                var $footerCell = this._createFooterCellForField(fieldName, this.options.fields[fieldName], colspan);
    	            $footerCell.data('fieldName', fieldName).appendTo($tr);
					colspan = 1;
				}else{
					colspan++;
        ...
        *************************************************************************/
        ...
        _createFooterCellForField: function (fieldName, field, colspan) {
			//dent-begin
            //return $('<th class="jtable-column-footer">' +
            return $('<th class="jtable-column-footer" colspan="'+colspan+'">' +
			//dent-end
                '<div class="jtable-column-footer-container"><span class="jtable-column-footer-text"></span></div></th>');
        }
		//dent-end
