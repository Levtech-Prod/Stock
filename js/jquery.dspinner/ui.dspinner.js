/*
	ui.dspinner - modified version for the dEnt project, modifications between //dent-begin and //dent-end
		- solve sizing problem - width
		- get rid of deprecated jQuery.browser
		- make it jquery ui 1.10 compatible
		- make wheel event work on jQuery 1.7+

/** 
 * @license jQuery UI dSpinner 1.20
 *
...
$.widget('ui.dspinner', {
	options: {
		...
		format: function(num, places) {
			...
			for (result = result.replace('.', options.point); regex.test(result) && options.group; result=result.replace(regex, '$1'+options.group+'$2')) {$.noop();}
	...
	// focused - this dspinner currently has the focus
	...
	_create: function() {
		...
		if (!input.is('input') || ((type != 'text') && (type != 'number'))) {
			console.error('Invalid target for ui.dspinner');
			return;
		}
	...
	_createButtons: function(input) {
		...
		var self = this,
			...
			box = true,//$.support.boxModel,// boxModel no longer supported by jQuery 2.x
			height = input.outerHeight(),
			rightMargin = self.oMargin = getMargin(input.css('margin-right')), // store original width and right margin for later destroy
			//dent-begin
			//solve control sizing problem
			//wrapper = self.wrapper = input.css({ width: (self.oWidth = (box ? input.width() : input.outerWidth())) - buttonWidth, 
			//wrapper = self.wrapper = input.css({ width: (self.oWidth = (box ? input.outerWidth() : input.width())) - buttonWidth - 1, 
			wrapper = self.wrapper = input.css({ width: 'calc(100% - '+buttonWidth+'px)', 
			//dent-end
												 marginRight: rightMargin + buttonWidth, textAlign: 'right' })
				.after('<span class="ui-dspinner ui-widget"></span>').next(),
			btnContainer = self.btnContainer = $(
				'<div class="ui-dspinner-buttons">' + 
					'<div class="ui-dspinner-up ui-dspinner-button ui-state-default ui-corner-tr"><span class="ui-icon '+options.upIconClass+'">&nbsp;</span></div>' + 
					'<div class="ui-dspinner-down ui-dspinner-button ui-state-default ui-corner-br"><span class="ui-icon '+options.downIconClass+'">&nbsp;</span></div>' + 
				'</div>'),
		...
		buttons = self.buttons = btnContainer.find('.ui-dspinner-button');
...
*/
		icons = buttons.find('.ui-icon.ui-icon-triangle-1-n');
		icons.css({ marginLeft: (buttons.innerWidth() - icons.width()) / 2 , marginTop:  (buttons.innerHeight() - icons.height()) / 2 +1});

		icons = buttons.find('.ui-icon.ui-icon-triangle-1-s');
		icons.css({ marginLeft: (buttons.innerWidth() - icons.width()) / 2 -1 , marginTop:  (buttons.innerHeight() - icons.height()) / 2 });
		...
		input.bind('keydown' + eventNamespace, function(e) {
					...
					switch (keyCode) {
						...
						//dEnt-begin - wired left right functionality
						/*case right:
						case left:
						...
						case end:
							...
							limit = self.options.max;
							//if (limit != null){ self._setValue(limit);}
							if ((limit != null) && (limit<99999)){ self._setValue(limit);}
							//dEnt-end
							return false;
					}
			...
			.bind('focus' + eventNamespace, function() {
					...
					if (msie){ selectAll(); } else { setTimeout(selectAll, 0); }
					self.focused = true;
					focusCtrl = self;
	...
	_mouseWheel: function(e) {
		//dEnt-begin // make it jquery ui 1.10 compatible
		//var self = $.data(this, 'dspinner');
		var self = $.data(this, 'ui-dspinner');
		//dEnt-end
		if (!self.options.disabled && self.focused && (focusCtrl === self)) {
			...
			self._doSpin(((self.extractDelta(e)) > 0 ? 1 : -1) * self.options.step);
			//dEnt-end
			return false;
		}
	},
	...
	_startSpin: function(dir, large) {
		...
		//dEnt-begin
			if (self.element.prop('readonly')||self.element.prop('disabled')){
				return;
			}
		//dEnt-end	
		// make sure any changes are posted
	...
	_validate: function(value) {
		...
		//dent-begin
		{
			...
			if (min==0){
				value = this.curvalue != null ? this.curvalue : 0; //richy separate condition when min==0
			}else{
				value = this.curvalue != null ? this.curvalue : min || max || 0; // must confirm not null in case just initializing and had blank value
			}	
		}
