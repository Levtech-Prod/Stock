...
class Statistics extends MY_Controller {
    ...
    public function list_statistics(){
        ...
        o.client_name, calc_qc_time(j.id) as qc_time, q.name as qc_type_name
                FROM jobs j
                left join materials m on (m.id = j.materialid)
                left join jobs_status js on (js.id = j.status)
                left join orders o on (o.id = j.order_id)
                left join qc_types q on (q.id = j.qc_type)
                WHERE 1=1 ";
        ...
        if($req['filter_qc_type']){
            $sql.=' AND (j.qc_type=?)';
            array_push($params,$req['filter_qc_type']);
        }
